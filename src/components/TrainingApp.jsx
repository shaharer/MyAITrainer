...
import { computeStats, getRecommendation } from "../hooks/useSmartStats";
...
const AI_TIPS = [
  ...
  "Snap a photo of any gym machine — AI identifies it instantly.",
  "Save exercises from videos to build a reference library of proper form cues.",
  "Ask Gena in the Coach tab for workout analysis or a custom training plan.",
...
export default function TrainingApp() {
  ...
  const [identifiedMachine, setIdentifiedMachine] = useState(null);
  const [photoWeight, setPhotoWeight] = useState("");
  const [photoReps, setPhotoReps] = useState("");
  ...
  const recommendation = useMemo(() => getRecommendation(workouts, plan), [workouts, plan]);
  ...
  const handleManualSubmit = async () => {
    ...
    showNotif(`✓ ${manualExercise.name}`);
    setManualExercise(null); setManualWeight(""); setManualReps(""); setManualSets("");
  };

  const handlePhotoIdentified = useCallback((machine) => {
    setIdentifiedMachine(machine);
    setCameraActive(false);
    setPhotoWeight(""); setPhotoReps(""); setPhotoSets("");
  }, []);

  const handlePhotoLog = async () => {
    if (!identifiedMachine || !photoWeight || !photoReps || !photoSets) return;
    await addWorkout({
      exercise: identifiedMachine.name, muscle: identifiedMachine.muscle,
      weight: Number(photoWeight), reps: Number(photoReps),
      sets: Number(photoSets), source: "photo",
    });
    showNotif(`✓ ${identifiedMachine.name}`);
    setIdentifiedMachine(null); setPhotoWeight(""); setPhotoReps(""); setPhotoSets("");
  };
  ...
  const handleVideoSave = async () => {
    ...
    showNotif(`✓ ${videoExercise.name}`);
    setVideoUrl(""); setVideoCreator(""); setVideoTitle(""); setVideoExercise(null);
  };
  ...
  const addToPlan = async (day) => {
    ...
    setPlanExercise(null); setEditingDay(null);
  };
  ...
  const applyRecommendation = async () => {
    if (!recommendation.suggestedExercises?.length) return;
    ...
    await updatePlan(newPlan);
    showNotif(`Added ${recommendation.suggestedExercises.length} exercises to ${todayDay}`);
  };

  const sourceIcon = (s) => (s === "photo" ? "📸" : s === "video" ? "🎬" : "✏️");
  const sourceLabel = (s) => (s === "photo" ? "Photo ID" : s === "video" ? "Video" : "Manual");
  ...
  return (
    <div style={{
          ...
          {showUserMenu && (
            <div style={{
              ...
              <button onClick={() => { logout(); setShowUserMenu(false); }} style={{
                ...
                padding: "12px 14px", color: "#FF6B6B", fontSize: 14, fontWeight: 600, cursor: "pointer",
                display: "flex", alignItems: "center", gap: 8,
              }}>
                <span style={{ fontSize: 16 }}>🚪</span> Sign out & switch account
              </button>
            </div>
          )}
        </div>
      ...
      <div style={{
        ...
        {["dashboard", "log", "plan", "coach", "library"].map((tab) => (
      ...
      <div style={{ padding: "20px 24px 120px", position: "relative", zIndex: 1 }}
        ...
        {activeTab === "dashboard" && (
          <div style={{ animation: "fadeIn 0.4s ease" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 20 }}>
              <div style={{ background: "#141419", borderRadius: 16, padding: "18px 16px", border: "1px solid #1E1E26" }}>
                <div style={{ fontSize: 10, color: "#666", fontFamily: "'Space Mono', monospace", letterSpacing: "1px", marginBottom: 10 }}>
                  STREAK
                </div>
                <div style={{ fontSize: 32, fontWeight: 700, color: stats.streak > 0 ? "#C6FF00" : "#444" }}>
                  {stats.streak}<span style={{ fontSize: 14, color: "#555", marginLeft: 4 }}>day{stats.streak !== 1 ? "s" : ""}</span>
                </div>
              </div>
              <div style={{ background: "#141419", borderRadius: 16, padding: "18px 16px", border: "1px solid #1E1E26" }}>
                <div style={{ fontSize: 10, color: "#666", fontFamily: "'Space Mono', monospace", letterSpacing: "1px", marginBottom: 10 }}>
                ...
                <div style={{ fontSize: 32, fontWeight: 700, color: "#E8E6E1" }}>
                  {stats.thisWeekDays}<span style={{ fontSize: 14, color: "#555", marginLeft: 4 }}>session{stats.thisWeekDays !== 1 ? "s" : ""}</span>
                </div>
              </div>
              <div style={{ background: "#141419", borderRadius: 16, padding: "18px 16px", border: "1px solid #1E1E26" }}>
                <div style={{ fontSize: 10, color: "#666", fontFamily: "'Space Mono', monospace", letterSpacing: "1px", marginBottom: 10 }}>
                ...
                <div style={{ fontSize: 32, fontWeight: 700, color: stats.prsThisWeek > 0 ? "#FFE66D" : "#444" }}>
                  {stats.prsThisWeek}<span style={{ fontSize: 14, color: "#555", marginLeft: 4 }}>new</span>
                </div>
              </div>
              <div style={{ background: "#141419", borderRadius: 16, padding: "18px 16px", border: "1px solid #1E1E26" }}>
                <div style={{ fontSize: 10, color: "#666", fontFamily: "'Space Mono', monospace", letterSpacing: "1px", marginBottom: 10 }}>
                ...
                <div style={{ fontSize: 32, fontWeight: 700, color: "#E8E6E1" }}>
                  {stats.uniqueExercises}<span style={{ fontSize: 14, color: "#555", marginLeft: 4 }}>unique</span>
                </div>
              </div>
            </div>
            ...
            {stats.topMuscle && (
              <div style={{
                ...
                <div style={{
                  ...
                  background: (muscleColors[stats.topMuscle] || "#888") + "22",
                  display: "flex", alignItems: "center", justifyContent: "center",
                  fontSize: 20, fontWeight: 700,
                  color: muscleColors[stats.topMuscle] || "#888",
                }}>
                  {stats.topMuscleSets}
                </div>
                <div>
                  <div style={{ fontSize: 14, fontWeight: 600 }}>
                    {stats.topMuscle} is your focus this week
                  </div>
                  <div style={{ fontSize: 12, color: "#666" }}>
                    {stats.topMuscleSets} sets — keep it balanced across groups
                  </div>
                </div>
              </div>
            )}
            ...
            {/* Muscle Distribution */}
            {Object.keys(stats.weekMuscles || {}).filter((m) => stats.weekMuscles[m] > 0).length > 0 && (
              <div style={{ background: "#141419", borderRadius: 16, padding: 20, border: "1px solid #1E1E26", marginBottom: 20 }}>
                <div style={{ fontSize: 10, color: "#666", fontFamily: "'Space Mono', monospace", letterSpacing: "2px", marginBottom: 16 }}>
                  WEEKLY MUSCLE BALANCE
                </div>
                {Object.entries(stats.weekMuscles).filter(([_, s]) => s > 0).sort((a, b) => b[1] - a[1]).map(([muscle, sets]) => {
                  const maxSets = Math.max(...Object.values(stats.weekMuscles));
                  return (
                    <div key={muscle} style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
                      ...
                      <div style={{ flex: 1, height: 24, background: "#1E1E26", borderRadius: 6, overflow: "hidden" }}>
                        <div style={{
                          height: "100%", width: `${(sets / maxSets) * 100}%`,
            ...
            {/* Badges */}
            <div style={{
              background: "#141419", borderRadius: 16, padding: 20,
              border: "1px solid #1E1E26", marginBottom: 20,
            }}>
              <div style={{ fontSize: 10, color: "#C6FF00", fontFamily: "'Space Mono', monospace", letterSpacing: "2px", marginBottom: 14 }}>
                BADGES
              </div>
              <Badges workouts={workouts} />
            </div>
        ...
        {activeTab === "log" && (
          <div style={{ animation: "fadeIn 0.4s ease" }}>
            ...
            <div style={{ fontSize: 13, color: "#666", marginBottom: 20 }}>Choose input method</div>

            {/* Today's Session Card */}
            <TodaySession workouts={workouts} onDelete={handleDeleteWorkout} />

            {/* Photo Capture button */}
            <button type="button" onClick={() => { setInputMode(inputMode === "photo" ? null : "photo"); setCameraActive(false); setIdentifiedMachine(null); }} style={{
              width: "100%", textAlign: "left",
              background: inputMode === "photo" ? "rgba(198,255,0,0.08)" : "#141419",
              border: `1px solid ${inputMode === "photo" ? "rgba(198,255,0,0.3)" : "#1E1E26"}`,
              ...
              <div>
                <div style={{ fontSize: 15, fontWeight: 700, color: "#E8E6E1" }}>Photo Identify</div>
                <div style={{ fontSize: 12, color: "#666", marginTop: 2 }}>Snap a photo → AI identifies the machine</div>
              </div>
            </button>
            ...
            {inputMode === "photo" && (
              <div style={{
                ...
                {!cameraActive && !identifiedMachine && (
                  <div style={{ textAlign: "center" }}>
                    <PhotoCapture
                      onIdentified={handlePhotoIdentified}
                ...
                {identifiedMachine && (
                  <div>
                    <div style={{
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: 10, color: "#C6FF00", fontFamily: "'Space Mono', monospace", letterSpacing: "1.5px" }}>AI IDENTIFIED</div>
                        <div style={{ fontSize: 18, fontWeight: 700 }}>{identifiedMachine.name}</div>
                        <div style={{ fontSize: 12, color: muscleColors[identifiedMachine.muscle] || "#888" }}>{identifiedMachine.muscle}</div>
                      </div>
                      <button type="button" onClick={() => setIdentifiedMachine(null)} style={{
                        background: "none", border: "1px solid #333", color: "#888",
                        padding: "6px 12px", borderRadius: 100, fontSize: 11, cursor: "pointer",
                      }}>Retake</button>
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 16 }}>
                      <NumberInput label="WEIGHT (LBS)" placeholder="135" value={photoWeight} onChange={setPhotoWeight} />
                      <NumberInput label="REPS" placeholder="10" value={photoReps} onChange={setPhotoReps} />
                      <NumberInput label="SETS" placeholder="3" value={photoSets} onChange={setPhotoSets} />
                    </div>
                    <button type="button" onClick={handlePhotoLog}
                      style={primaryBtn(!photoWeight || !photoReps || !photoSets)}>
                      Log This Set →
                    </button>
                  </div>
                )}
            ...
            {inputMode === "video" && (
              <div style={{
                ...
                <div style={{ marginBottom: 14 }}>
                  ...
                  <ExercisePicker value={videoExercise?.name || null} muscle={videoExercise?.muscle || videoMuscle}
                    onSelect={(ex) => { if (ex) { setVideoExercise(ex); setVideoMuscle(ex.muscle); } else setVideoExercise(null); }}
                    knownExercises={knownExercises} placeholder="Search or type exercise..." />
                </div>
                {videoExercise?.isNew && (
                  <div style={{ marginBottom: 14 }}>
                    <span style={labelStyle}>MUSCLE GROUP</span>
                    <MuscleSelect value={videoMuscle} onChange={(m) => { setVideoMuscle(m); setVideoExercise((p) => p ? { ...p, muscle: m } : p); }} />
                  </div>
                )}
            ...
            <div style={{ background: "#141419", borderRadius: 20, padding: 24, border: "1px solid #1E1E26" }}>
              ...
              <div style={{ marginBottom: 14 }}>
                <span style={labelStyle}>EXERCISE</span>
                <ExercisePicker value={manualExercise?.name || null} muscle={manualExercise?.muscle || manualMuscle}
                  onSelect={(ex) => { if (ex) { setManualExercise(ex); setManualMuscle(ex.muscle); } else setManualExercise(null); }}
                  knownExercises={knownExercises} placeholder="Search or type exercise name..." />
              </div>
              {manualExercise?.isNew && (
                <div style={{ marginBottom: 14 }}>
                  <span style={labelStyle}>MUSCLE GROUP</span>
                  <MuscleSelect value={manualMuscle} onChange={(m) => { setManualMuscle(m); setManualExercise((p) => p ? { ...p, muscle: m } : p); }} />
                </div>
              )}
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 16 }}>
        ...
        {activeTab === "plan" && (
          <div style={{ animation: "fadeIn 0.4s ease" }}>
            ...
            <div style={{ fontSize: 13, color: "#666", marginBottom: 20 }}>AI-powered recommendations + custom planning</div>

            {/* Smart Recommendation Card */}
            <div style={{
              background: "rgba(198,255,0,0.04)", borderRadius: 20, padding: 20,
              border: "1px solid rgba(198,255,0,0.15)", marginBottom: 24,
            }}>
              <div style={{ fontSize: 10, color: "#C6FF00", fontFamily: "'Space Mono', monospace", letterSpacing: "2px", marginBottom: 10 }}>
                AI RECOMMENDATION
              </div>
              <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 6, color: "#E8E6E1" }}>
                {recommendation.title}
              </div>
              <div style={{ fontSize: 13, color: "#B8B5AD", lineHeight: 1.6, marginBottom: 14 }}>
                {recommendation.message}
              </div>

              {recommendation.suggestedMuscles?.length > 0 && (
                <div style={{ display: "flex", gap: 6, marginBottom: 14, flexWrap: "wrap" }}>
                  {recommendation.suggestedMuscles.map((m) => (
                    <span key={m} style={{
              ...
              {recommendation.suggestedExercises?.length > 0 && (
                <div>
                  <div style={{ fontSize: 11, color: "#666", marginBottom: 8 }}>Suggested exercises:</div>
                  {recommendation.suggestedExercises.map((ex, i) => (
                    <div key={i} style={{
                      display: "flex", alignItems: "center", gap: 8,
                      padding: "6px 10px", background: "rgba(0,0,0,0.2)", borderRadius: 8, marginBottom: 4,
                    }}>
                      <div style={{ width: 6, height: 6, borderRadius: "50%", background: muscleColors[ex.muscle] || "#888" }} />
                      <div style={{ fontSize: 13, color: "#B8B5AD" }}>{ex.exercise}</div>
                      <div style={{ fontSize: 10, color: "#555", marginLeft: "auto" }}>{ex.muscle}</div>
                    </div>
                  ))}
                  <button type="button" onClick={applyRecommendation} style={{
                    width: "100%", background: "rgba(198,255,0,0.15)", color: "#C6FF00",
                    border: "1px solid rgba(198,255,0,0.3)",
                    padding: "10px", borderRadius: 100, fontSize: 13,
                  ...
                  {isEditing && (
                    <div style={{ marginTop: 10, padding: 14, background: "#0A0A0F", borderRadius: 12 }}>
                      <div style={{ marginBottom: 10 }}>
                        <ExercisePicker value={planExercise?.name || null} muscle={planExercise?.muscle || planMuscle}
                          onSelect={(ex) => { if (ex) { setPlanExercise(ex); setPlanMuscle(ex.muscle); } else setPlanExercise(null); }}
                          knownExercises={knownExercises} placeholder="Search or add exercise..." />
                      </div>
                      {planExercise?.isNew && (
                        <div style={{ marginBottom: 10 }}>
                          <MuscleSelect value={planMuscle} onChange={(m) => { setPlanMuscle(m); setPlanExercise((p) => p ? { ...p, muscle: m } : p); }} />
                        </div>
                      )}
        ...
        {activeTab === "coach" && (
          <div style={{ animation: "fadeIn 0.4s ease" }}>
            <div style={{ fontSize: 20, fontWeight: 700, marginBottom: 4 }}>AI Coach</div>
            <div style={{ fontSize: 13, color: "#666", marginBottom: 20 }}>Ask questions, analyze workouts, or generate plans</div>
            <AIChat workouts={workouts} />
          </div>
        )}
        ...
        {activeTab === "library" && (
          <div style={{ animation: "fadeIn 0.4s ease" }}>
                    ...
                    return (
                      <div key={ex} style={{
                        ...
                        <div style={{ textAlign: "right", flexShrink: 0 }}>
                          ...
                          <div style={{ fontSize: 10, color: "#555" }}>PR</div>
                        </div>
                      </div>
                    );
                  })}
            ...
            {videos.length > 0 && (
              <div>
                <div style={{ fontSize: 10, fontFamily: "'Space Mono', monospace", letterSpacing: "2px", color: "#4ECDC4", marginBottom: 10 }}>
                  SAVED VIDEOS
                </div>
                {videos.map((v) => (
                  <div key={v.id} style={{
